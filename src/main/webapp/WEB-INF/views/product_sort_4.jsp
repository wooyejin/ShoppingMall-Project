						...
						<div class="flex-w">
							<div class="rs2-select2 bo4 of-hidden w-size12 m-t-5 m-b-5 m-r-10"style="background:black;">
								<select class="selection-2" id="sortSelect" name="sorting"style="background:black;" onchange="sort(this.value);">
									<option style="background:black" value="4">높은가격 순</option>
									<option value="1" >인기 순</option>
									<option value="2">리뷰 순</option>
									<option value="3">낮은가격 순</option>
									<option value="0">최근게시물 순</option>
								</select>
							</div>
...
<c:if test="${ mainPi.currentPage ne 1 }">
                  <c:url var="before" value="sort1.do">
                  <c:param name="product1" value="<%=upPage %>"/>
                  <c:param name="sort" value="4"/>
                  ...
                  <img src="/ot/resources/images/btn_page_prev.gif" alt="이전 페이지">
                     ...
                     <c:if test="${ p ne mainPi.currentPage }">
                        <c:url var="pagination" value="sort1.do">
                           <c:param name="currentPage" value="${ p }"/>
                     </c:url>
                     <a href="http://localhost:8888/ot/sort1.do?product1=<%=upPage %>&sort=4&currentPage=${p }" class="item-pagination flex-c-m trans-0-4">
                     ${ p }</a>
                  </c:if>
               </c:forEach>
               
               <c:if test="${ mainPi.currentPage ne mainPi.maxPage }">
               <c:url var="after" value="sort1.do">
               		 <c:param name="product1" value="<%=upPage %>"/>
               		 <c:param name="sort" value="4"/>
                     <c:param name="currentPage" value="${mainPi.currentPage +1 }"/>
                  </c:url>
